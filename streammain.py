...
with credentials:
    with st.form("Form1"):
        ...
        team = colb.selectbox("Select your role",options=["my role","Analyst","Account Manager","Trader","CS","Other"])
        submitted1 = st.form_submit_button('Submit')
        if (len(name)<3 or name=="my name" or any(map(str.isdigit, name)) or team=="my role"):
            st.info("Please enter valid user info")
            st.stop()
        current_time = datetime.datetime.now()

with testing:
    
    st.success("Welcome {}! Please enter the campaign details to continue".format(name))
    ...
    if(col2.button("Submit")==False):
        st.info("Please submit your input")
        st.stop()
    ...
    if(conv_rate_b > conv_rate_a):
        ...
        se_diff = pow((pow(se_a,2)+pow(se_b,2)),0.5)
        z_score = (conv_rate_a - conv_rate_b)/se_diff
        relative_uplift = (conv_rate_a - conv_rate_b)/conv_rate_b
    
    else:
        st.warning("The campaigns have the same performance")
        st.stop()
    ...
    if(p_value<alpha):
        if(conv_rate_a>conv_rate_b):
            st.balloons()
            st.success("Your results are statistically significant! You can be {} sure that the uplift of {}% observed in Group A is a consequence of the changes you made and not a result of random chance.".format(confidence,round(relative_uplift*100,2)))
        elif(conv_rate_b>conv_rate_a):
            st.balloons()
            st.success("Your results are statistically significant! You can be {} sure that the uplift of {}% observed in Group B is a consequence of the changes you made and not a result of random chance.".format(confidence,round(relative_uplift*100,2)))
    ...
    if(hypothesis == "One Tailed"):
        if(conv_rate_b > conv_rate_a):
            ...
            z_crit = stats.norm.ppf(1-alpha)
            X_c = (z_crit*se_b)+conv_rate_b
            power = 1-norm(conv_rate_a, se_a).cdf(np.absolute(X_c))
...
###############################################################################################################################################################
    ...
    class A_One:
      ...
      def plot(mean, std, lower_bound=None, upper_bound=None, resolution=None,
        ...
        legend_label = legend_label or "Group A"
    ...
    class B_One:
      ...
      def plot(mean, std, lower_bound=None, upper_bound=None, resolution=None,
        ...
        legend_label = legend_label or "Group B"

        X = np.linspace(lower_bound, upper_bound, resolution)
        dist_X = B_One._distribution(X, mean, std)
    ...
    class B_Two:
      ...
      def plot(mean, std, lower_bound=None, upper_bound=None, resolution=None,
        ...
        X = np.linspace(lower_bound, upper_bound, resolution)
        dist_X = B_Two._distribution(X, mean, std)
...
###############################################################################################################################################################
    ...
    if(hypothesis == "One Tailed"):
        ...
        plot = B_One.plot(conv_rate_b, se_b)
    elif(hypothesis == "Two Tailed"):
        plot = A_Two.plot(conv_rate_a, se_a)
        plot = B_Two.plot(conv_rate_b, se_b)
    ...
    fields = [current_time, name, team, imp_a, imp_b, conv_a, conv_b, hypothesis, confidence]
    with open('Database.csv','a', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(fields)
    
    col11.write("Conversion Rate Group A")
    col11.write(str(round(conv_rate_a*100,3))+"%")
    col12.write("Conversion Rate Group B")
